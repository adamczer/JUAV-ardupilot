...
bool ModeAuto::juav_is_nav_guided_enabled(){
#if NAV_GUIDED == ENABLED
    return true;
#else
    return false;
...
float juav_mode_auto_target_yaw_rate = 0;
float ModeAuto::juavGetTargetYawRate() {
    return juav_mode_auto_target_yaw_rate;
}
void ModeAuto::juav_wp_run_prior_to_attitude_control()
{
    // process pilot's yaw input
    juav_mode_auto_target_yaw_rate = 0;
    if (!copter.failsafe.radio) {
        // get pilot's desired yaw rate
        juav_mode_auto_target_yaw_rate = get_pilot_desired_yaw_rate(channel_yaw->get_control_in());
        if (!is_zero(juav_mode_auto_target_yaw_rate)) {
            auto_yaw.set_mode(AUTO_YAW_HOLD);
        }
    }
...
void ModeAuto::juav_spline_run_prior_to_attitude()
{
    // if not armed set throttle to zero and exit immediately
    if (is_disarmed_or_landed()) {
        make_safe_spool_down();
...
void ModeAuto::juav_circle_run_prior_to_attitude()
{
    // call circle controller
    copter.circle_nav->update();
...
void ModeAuto::juav_loiter_run_prior_to_attitude()
{
    // if not armed set throttle to zero and exit immediately
    if (is_disarmed_or_landed()) {
        make_safe_spool_down();
