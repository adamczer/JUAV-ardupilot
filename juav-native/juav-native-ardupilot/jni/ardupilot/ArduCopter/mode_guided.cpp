...
#endif
float juav_guided_target_yaw_rate = 0;
void ModeGuided::juavGuidedPosControlPriorToAttitude() {
    // process pilot's yaw input
    juav_guided_target_yaw_rate =0;
    if (!copter.failsafe.radio) {
        // get pilot's desired yaw rate
        juav_guided_target_yaw_rate = get_pilot_desired_yaw_rate(channel_yaw->get_control_in());
        if (!is_zero(juav_guided_target_yaw_rate)) {
            auto_yaw.set_mode(AUTO_YAW_HOLD);
        }
    }
...
float ModeGuided::juavGuidedGetTargetYawRate() {
    return juav_guided_target_yaw_rate;
}

void ModeGuided::juavGuidedVelControlRunPriorToAttitude() {
    // process pilot's yaw input
    juav_guided_target_yaw_rate =0;
        if (!copter.failsafe.radio) {
            // get pilot's desired yaw rate
...
void ModeGuided::juavGuidedPosVelControlRunPriorToAttitude() {
...
float juav_pitch_in;
float juav_yaw_in;
float juav_yaw_rate_in;
float juav_climb_rate_cms;
void ModeGuided::juavGuidedAngleControlRunPriorToAttitude() {
    // constrain desired lean angles
        juav_roll_in = guided_angle_state.roll_cd;
        juav_pitch_in = guided_angle_state.pitch_cd;
        float total_in = norm(juav_roll_in, juav_pitch_in);
        float angle_max = MIN(attitude_control->get_althold_lean_angle_max(), copter.aparm.angle_max);
        if (total_in > angle_max) {
            float ratio = angle_max / total_in;
            juav_roll_in *= ratio;
        ...
        // wrap yaw request
        juav_yaw_in = wrap_180_cd(guided_angle_state.yaw_cd);
        juav_yaw_rate_in = wrap_180_cd(guided_angle_state.yaw_rate_cds);

        // constrain climb rate
        juav_climb_rate_cms = constrain_float(guided_angle_state.climb_rate_cms, -fabsf(wp_nav->get_default_speed_down()), wp_nav->get_default_speed_up());

        // get avoidance adjusted climb rate
        juav_climb_rate_cms = get_avoidance_adjusted_climbrate(juav_climb_rate_cms);
        ...
        if (tnow - guided_angle_state.update_time_ms > GUIDED_ATTITUDE_TIMEOUT_MS) {
            juav_roll_in = 0.0f;
            juav_pitch_in = 0.0f;
            juav_climb_rate_cms = 0.0f;
            juav_yaw_rate_in = 0.0f;
        }
...
float ModeGuided::juavGuidedGetAngleControlRunRollIn() {
    return juav_roll_in;
}

float ModeGuided::juavGuidedGetAngleControlRunPitchIn() {
    return juav_pitch_in;
}

bool ModeGuided::juavGuidedIsAngleStateUseYawRate() {
    return guided_angle_state.use_yaw_rate;
}

float ModeGuided::juavGuidedGetAngleControlRunYawRateIn() {
    return juav_yaw_rate_in;
}

float ModeGuided::juavGuidedGetAngleControlRunYawIn() {
    return juav_yaw_in;
}

void ModeGuided::juavGuidedAngleControlRunAfterAttitude() {
    // call position controller
        pos_control->set_alt_target_from_climb_rate_ff(juav_climb_rate_cms, G_Dt, false);
        pos_control->update_z_controller();
}
